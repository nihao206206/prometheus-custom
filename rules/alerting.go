...
func (r *AlertingRule) Eval(ctx context.Context, ts time.Time, query QueryFunc, externalURL *url.URL) (promql.Vector, error) {
	...
	blog.Debugf("### alerting.go Eval res=%s, ts=%d,unix=%d,vector=%s \n", res,ts,ts.Unix(),r.vector.String())
	r.mtx.Lock()
	defer r.mtx.Unlock()
...
func (r *AlertingRule) currentAlerts() []*Alert {
	...
	blog.Debugf("### alerting.go currentAlerts active=%s ,len=%d \n", r.active,len(r.active))
	for _, a := range r.active {
		blog.Debugf("### alerting.go currentAlerts a=%s \n", a)
		anew := *a
