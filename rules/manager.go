...
type EvalStep struct {
	SampleTime    model.Time
	OldSampleTime model.Time
	Step          time.Duration
}
...
func (g *Group) Eval(ctx context.Context, ts time.Time) {
	for i, rule := range g.rules {
		...
		func(i int, rule Rule) {
			...
			current := model.Time(ts.Unix()*1000)
			// var current model.Time
			if sampletime, isexist := models.SeriesAlertDesc.TimesMap.Get(rule.Name()); isexist {
				if sampletime.(*EvalStep).SampleTime != sampletime.(*EvalStep).OldSampleTime {
					current = sampletime.(*EvalStep).SampleTime.Add(1000000000)
					t := &EvalStep{
						SampleTime:    current,
						OldSampleTime: current,
						Step:          g.interval,
					}
					models.SeriesAlertDesc.TimesMap.Set(rule.Name(), t)
					if s1, ist := models.SeriesAlertDesc.TimesMap.Get(rule.Name()); ist {
						blog.Debugf("### manager.go Eval1 s1.SampleTime=%d ,s1.OldSampleTime=%d \n", s1.(*EvalStep).SampleTime, s1.(*EvalStep).OldSampleTime)
					}
					
					blog.Debugf("### manager.go Eval1 t.SampleTime=%d ,Step=%d,OldSampleTime=%d,Name=%s,sampletime.Unix=%d \n", t.SampleTime, t.Step,t.OldSampleTime,rule.Name(),sampletime.(*EvalStep).SampleTime.Unix())
					blog.Debugf("### manager.go Eval1 SampleTime=%d ,OldSampleTime=%d \n", sampletime.(*EvalStep).SampleTime, sampletime.(*EvalStep).OldSampleTime)
				} else {
					current = sampletime.(*EvalStep).SampleTime.Add(sampletime.(*EvalStep).Step)
					t := &EvalStep{
						SampleTime:    current,
						OldSampleTime: current,
						Step:          sampletime.(*EvalStep).Step,
					}
					models.SeriesAlertDesc.TimesMap.Set(rule.Name(), t)
					blog.Debugf("### manager.go Eval2 t.SampleTime=%d ,Step=%d,OldSampleTime=%d,Name=%s,sampletime.Unix=%d \n", t.SampleTime, t.Step,t.OldSampleTime,rule.Name(),sampletime.(*EvalStep).SampleTime.Unix())
					blog.Debugf("### manager.go Eval2 SampleTime=%d ,OldSampleTime=%d \n", sampletime.(*EvalStep).SampleTime, sampletime.(*EvalStep).OldSampleTime)
				}

				// models.SeriesAlertDesc.TimesMap.Set(rule.Name(), current.Add(time.Second*1))
				// blog.Debugf("### manager.go rule isexist sampletime=%d ,now=%d ", sampletime, now)
				//models.SeriesAlertDesc.TimesMap[rule.Name()] = -1
			}
			subtime := int64(current/1000)-ts.Unix()
			tnow := ts.Add(time.Duration(subtime))
			// time.Unix(int64(current), 0)
			blog.Debugf("### manager.go Eval current=%d ,currentUnix=%d,currentUnixNano=%d,ts=%d ,Unix1=%d,Unix2=%d,interval=%d ,tsUnix=%d,Name=%s,ts.Add=%d ,subtime=%d  \n", current, current.Unix(),current.UnixNano(),ts,tnow,tnow.Unix(),g.interval,ts.Unix(),rule.Name(),ts.Add(time.Duration(int64(current)-ts.Unix())),subtime)
			
			vector, err := rule.Eval(ctx, time.Unix(current.Unix(), 0), g.opts.QueryFunc, g.opts.ExternalURL)
			if err != nil {
			...
			if ar, ok := rule.(*AlertingRule); ok {
				blog.Debugf("### manager.go Eval vector=%s ,Name=%s \n", ar.vector.String(),rule.Name())
			...
			for _, s := range vector {
				blog.Debugf("### manager.go Eval Metric=%s,T=%d,v=%d,Name=%s \n", s.Metric,s.T,s.V,rule.Name())
...
func (m *Manager) loadGroups(interval time.Duration, filenames ...string) (map[string]*Group, []error) {
	...
	blog.Debugf("### manager.go rules loadGroups len=%d,rulelist=%s ", rulelist.Len(),rulelist)
	itv := interval
	...
	if len(rules) > 0 {
	    groups[groupKey(rName, rFn)] = NewGroup(rName, rFn, itv, rules, m.opts)
	}
