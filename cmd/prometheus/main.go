...
func init() {
	...
	models.Initialization("root:123456@tcp(192.168.43.107:3306)/prometheus_2")
	blog.Debug("start prometheus1")
}
...
func main() {
	...
	reloaders := []func(cfg *config.Config) error{
		...
		func(cfg *config.Config) error {
			...
			blog.Debugf("init discoveryManagerNotify ApplyConfig \n")
...
func sendAlerts(n *notifier.Notifier, externalURL string) rules.NotifyFunc {
	return func(ctx context.Context, expr string, alerts ...*rules.Alert) error {
		...
		blog.Debugf("### start main.go sendAlerts expr=%s \n",expr)
		blog.Debugf("### start main.go sendAlerts alerts=%s ,len=%d \n",alerts,len(alerts))
		for _, alert := range alerts {
			blog.Debugf("### main.go sendAlerts alert=%s \n", alert)
