...
func (m *Manager) ApplyConfig(cfg map[string]sd_config.ServiceDiscoveryConfig) error {
	...
	m.cancelDiscoverers()
	blog.Debugf("discovery Manager ApplyConfig cfg=%s \n",cfg)
	for name, scfg := range cfg {
		blog.Debugf("discovery Manager ApplyConfig name=%s ,scfg=%s \n",name,scfg)
		for provName, prov := range m.providersFromConfig(scfg) {
			blog.Debugf("discovery Manager ApplyConfig provName=%s ,prov=%s \n",provName,prov)
...
func (m *Manager) updateGroup(poolKey poolKey, tgs []*targetgroup.Group) {
	...
	for _, tg := range tgs {
		if tg != nil { // Some Discoverers send nil target group so need to check for it to avoid panics.
			...
			blog.Debugf("Manager updateGroup poolKey=%s ,Source=%s Targets=%s,Labels=%s ,setName=%s ,provider=%s \n",poolKey,tg.Source,tg.Targets,tg.Labels,poolKey.setName,poolKey.provider)
		}
	}
}
