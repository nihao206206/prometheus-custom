...
func (n *Notifier) ApplyConfig(conf *config.Config) error {
	...
	blog.Debugf("Notifier ApplyConfig AlertmanagerConfigs=%s \n",conf.AlertingConfig.AlertmanagerConfigs)
	for _, cfg := range conf.AlertingConfig.AlertmanagerConfigs {
		ams, err := newAlertmanagerSet(cfg, n.logger)
		blog.Debugf("Notifier ApplyConfig ams0=%s \n",ams)
		if err != nil {
			return err
		}

		ams.metrics = n.metrics
		blog.Debugf("Notifier ApplyConfig ams1=%s \n",ams)
		...
		blog.Debugf("Notifier ApplyConfig b=%s \n",b)
...
func (n *Notifier) reload(tgs map[string][]*targetgroup.Group) {
	n.mtx.Lock()
	defer n.mtx.Unlock()
	blog.Debugf("Notifier reload tgs=%s \n",tgs)
	for id, tgroup := range tgs {
		blog.Debugf("Notifier reload id=%s, tgroup=%s \n",id,tgroup)
...
func (n *Notifier) Send(alerts ...*Alert) {
	...
	for _, a := range alerts {
		blog.Debugf("Notifier start Send Alert a.Labels=%s \n",a.Labels)
		...
		blog.Debugf("Notifier end Send Alert a.Labels=%s \n",a.Labels)
	}
...
func (n *Notifier) relabelAlerts(alerts []*Alert) []*Alert {
	...
	for _, alert := range alerts {
		blog.Debugf("Notifier start relabelAlerts Alert alert.Labels=%s \n",alert.Labels)
		
		labels := relabel.Process(alert.Labels, n.opts.RelabelConfigs...)
		if labels != nil {
			alert.Labels = labels
			blog.Debugf("Notifier end relabelAlerts Alert alert.Labels=%s \n",alert.Labels)
...
func (n *Notifier) sendAll(alerts ...*Alert) bool {
	begin := time.Now()
	blog.Debugf("Notifier sendAll alerts=%s \n",alerts)
...
func newAlertmanagerSet(cfg *config.AlertmanagerConfig, logger log.Logger) (*alertmanagerSet, error) {
	blog.Debugf("Notifier newAlertmanagerSet cfg.HTTPClientConfig=%s \n",cfg.HTTPClientConfig)
...
func alertmanagerFromGroup(tg *targetgroup.Group, cfg *config.AlertmanagerConfig) ([]alertmanager, error) {
	...
	for _, tlset := range tg.Targets {
		...
		blog.Debugf("Notifier alertmanagerFromGroup lbls=%s \n",lbls)
		lset := relabel.Process(labels.New(lbls...), cfg.RelabelConfigs...)
		if lset == nil {
			continue
		}
		blog.Debugf("Notifier alertmanagerFromGroup lset=%s \n",lset)
		lb := labels.NewBuilder(lset)
		blog.Debugf("Notifier alertmanagerFromGroup lb1=%s \n",lb)
		...
		blog.Debugf("Notifier alertmanagerFromGroup addr=%s ,AddressLabel=%s \n",addr,model.AddressLabel)
		...
		blog.Debugf("Notifier alertmanagerFromGroup  end lset=%s \n",lset)
		
		// Meta labels are deleted after relabelling. Other internal labels propagate to
		// the target which decides whether they will be part of their label set.
		blog.Debugf("Notifier alertmanagerFromGroup  end model.MetaLabelPrefix=%s \n",model.MetaLabelPrefix)
		for _, l := range lset {
			blog.Debugf("Notifier alertmanagerFromGroup  end lName=%s \n",l.Name)
			if strings.HasPrefix(l.Name, model.MetaLabelPrefix) {
				lb.Del(l.Name)
			}
		}

		blog.Debugf("Notifier alertmanagerFromGroup  end123 lset=%s \n",lset)
