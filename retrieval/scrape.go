...
func (sp *scrapePool) Sync(tgs []*targetgroup.Group) {
	...
	for _, tg := range tgs {
		blog.Debugf("scrapePool Sync tg=%s ,config=%s \n",tg,sp.config)
		...
		for _, t := range targets {
			blog.Debugf("scrapePool Sync targets t=%s ,name=%s \n",t,t.String())
			if t.Labels().Len() > 0 {
				all = append(all, t)
...
func (sp *scrapePool) sync(targets []*Target) {
	...
	for _, t := range targets {
		...
		if _, ok := sp.targets[hash]; !ok {
			s := &targetScraper{Target: t, client: sp.client, timeout: timeout}
			l := sp.newLoop(t, s)
			blog.Debugf("scrapePool s url=%s \n",s.URL().String())
			sp.targets[hash] = t
			sp.loops[hash] = l
...
func (s *targetScraper) delete(ctx context.Context) error {
	//删除
	blog.Debugf("targetScraper scrape1 URL=%s \n",s.URL().String()+"/job/chenxjob")
	if !strings.Contains(s.URL().String(), "9090") {
		// delCtx, cancel := context.WithCancel(ctx)
		blog.Debugf("targetScraper scrape2 URL=%s \n",s.URL().String()+"/job/chenxjob")
		reqDelete, errDelete := http.NewRequest("DELETE", s.URL().String()+"/job/chenxjob", nil)
		if errDelete != nil {
			return errDelete
		}
		reqDelete.Header.Add("Accept", acceptHeader)
		respDelete, errDelete := ctxhttp.Do(ctx, s.client, reqDelete)
		if errDelete != nil {
			return errDelete
		}
		if respDelete.StatusCode != http.StatusAccepted {
			return fmt.Errorf("server returned HTTP status %s", respDelete.Status)
		}
		//cancel()
...
func (sl *scrapeLoop) matchMetric(s *model.Sample) (string, bool) {
	// var (
	// 	ismatch, retbool bool
	// 	retString        string
	// )
	ismatch := true
	retbool := false
	...
	blog.Debugf("matchMetric name:%s", name)
	if val, isexist := models.SeriesAlertDesc.DimensMap.Get(string(name)); isexist {
		blog.Debugf("matchMetric name is exist:%s", name)
		blog.Debugf("matchMetric1 ismatch=%v", ismatch)
		msg := s.Metric.GetFormatMetricMsg()
		for _, dimen := range val.([]models.AlertDescribe) {
			ismatch = true
			blog.Debugf("matchMetric2 id=%s,ismatch=%v", dimen.Id, ismatch)
			for _, text := range dimen.DimenArry {
				blog.Debugf("matchMetric msg=%s,text=%s", msg, text)
				if !strings.Contains(msg, text) {
					ismatch = false
					blog.Debugf("matchMetric msg=%s,text=%s,ismatch=%v", msg, text, ismatch)
					break
				}
			}
			blog.Debugf("matchMetric3 id=%s,ismatch=%v", dimen.Id, ismatch)
			if ismatch {
				retString += dimen.Id + "|"
				blog.Debugf("matchMetric ismatch=%v,retString=%s", ismatch, retString)
				retbool = true
			}
		}
	}
...
func (sl *scrapeLoop) setMetricTimestamp(lset labels.Labels, t int64, v float64) (err error) {
	var(
		alert_name = ""
		isexist    = false
		nameArray  []string
	...
	for _,value := range lset {
		metric[model.LabelName(value.Name)]=model.LabelValue(value.Value)
	}
	...
	if alert_name, isexist = sl.matchMetric(s); isexist {
		// alertName :=
		nameArray = strings.Split(alert_name, "|")
		for _, v := range nameArray {
			...
			models.SeriesAlertDesc.TimesMap.Set(v, t)
			blog.Debugf("### scrape.go append isexist alert_name=%s \n", v)
		}
	}
...
loop:
	for p.Next() {
		...
		if ok {
			...
			blog.Debugf("scrapeLoop AddFast lset=%s,t=%d ,v=%d ,ref=%f,err=%s,err=%d \n",ce.lset,t,v,ce.ref,err,err)
			switch err {
			case nil:
				if tp == nil {
			...
			if err == nil{
				sl.setMetricTimestamp(ce.lset,t,v)
			}
		}
		if !ok {
			...
			blog.Debugf("scrapeLoop append1 lset=%s,mets=%s \n",lset,mets)
			lset = sl.sampleMutator(lset)

			blog.Debugf("scrapeLoop append2 lset=%s,mets=%s \n",lset,mets)
			...
			blog.Debugf("scrapeLoop Add lset=%s,t=%d ,v=%f ,ref=%d \n",lset,t,v,ref)
			...
			if err == nil{
				sl.setMetricTimestamp(lset,t,v)
			}
			if tp == nil {
	...
	blog.Debugf("### scrape.go append Rollback err=%s \n", err)
	if err != nil {
		app.Rollback()
		return total, added, err
	}
	blog.Debugf("### scrape.go append start app.Commit \n")
	if err := app.Commit(); err != nil {
		blog.Debugf("### scrape.go append total=%d, err=%s \n",total,err)
		return total, added, err
	}
